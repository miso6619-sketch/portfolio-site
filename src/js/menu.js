export const navItems = [
  {
    title: "프로필",
    path: "/about",
    children: [],
  },
  {
    title: "영상편집",
    path: "/video",
    children: [
      { title: "웹 영상", sectionId: "web-video" },
      { title: "콘셉트 영상", sectionId: "concept-video" },
      { title: "캠페인 영상", sectionId: "campaign-video" },
      { title: "클래스워크", sectionId: "class-video" },
    ],
  },
  {
    title: "웹페이지",
    path: "/web",
    children: [
      { title: "프로젝트 01", sectionId: "web-project-01" },
      { title: "프로젝트 02", sectionId: "web-project-02" },
      { title: "프로젝트 03", sectionId: "web-project-03" },
      { title: "프로젝트 04", sectionId: "web-project-04" },
      { title: "프로젝트 05", sectionId: "web-project-05" },
      { title: "프로젝트 06", sectionId: "web-project-06" },
      { title: "프로젝트 07", sectionId: "web-project-07" },
    ],
  },
  {
    title: "디자인",
    path: "/design",
    children: [
      { title: "작업 01", sectionId: "design-01" },
      { title: "작업 02", sectionId: "design-02" },
      { title: "작업 03", sectionId: "design-03" },
      { title: "작업 04", sectionId: "design-04" },
      { title: "작업 05", sectionId: "design-05" },
      { title: "작업 06", sectionId: "design-06" },
      { title: "작업 07", sectionId: "design-07" },
      { title: "작업 08", sectionId: "design-08" },
      { title: "작업 09", sectionId: "design-09" },
      { title: "작업 10", sectionId: "design-10" },
      { title: "작업 11", sectionId: "design-11" },
      { title: "작업 12", sectionId: "design-12" },
      { title: "작업 13", sectionId: "design-13" },
      { title: "작업 14", sectionId: "design-14" },
      { title: "작업 15", sectionId: "design-15" },
    ],
  },
];