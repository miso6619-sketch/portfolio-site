const webImages = import.meta.glob("../assets/images/web/*", {
  eager: true,
  query: "?url",
  import: "default",
});

const getWebImage = (fileName) => {
  return webImages[`../assets/images/web/${fileName}`];
};

export const webProjects = [
  {
    id: "web-project-05",
    title: "Perfume Library",
    desc: "향수와 바디 케어 브랜드 콘셉트의 사이트로, Photoshop으로 디자인 후 HTML, CSS, JavaScript, jQuery를 활용해 초기에 제작한 웹사이트 메인페이지입니다.",
    notionUrl: "https://www.notion.so/Perfume-Library-34e51f8c4166801f84cdf3c14c6c3d75?source=copy_link",
    siteUrl: "https://miso6619-sketch.github.io/Perfume-Library/",
    image: getWebImage("web-project-05.png"),
    imageAlt: "Perfume Library 풀 페이지 캡쳐 이미지",
  },
  {
    id: "web-project-01",
    title: "THE SOUND PROJECT",
    desc: "HTML, CSS, JavaScript 기반으로 제작하고 jQuery와 Swiper를 활용해 인터랙션을 구현한 오디오 브랜드 콘셉트 웹사이트입니다.",
    notionUrl: "https://www.notion.so/THE-SOUND-PROJECT-35051f8c41668064b0f4fe75a7bda5a7?source=copy_link",
    siteUrl: "https://miso6619-sketch.github.io/THE-SOUND-PROJECT/index.html",
    image: getWebImage("web-project-01.png"),
    imageAlt: "THE SOUND PROJECT 풀 페이지 캡쳐 이미지",
  },
  {
    id: "web-project-02",
    title: "REAL OR FAKE",
    desc: "개인정보 유출과 딥페이크 위험성을 주제로 제작한 개인정보보호 캠페인 웹사이트입니다.",
    notionUrl: "https://www.notion.so/REAL-or-FAKE-35051f8c4166809fabf2fc5d9f726efa?source=copy_link",
    siteUrl: "https://miso6619-sketch.github.io/privacy-campaign/",
    image: getWebImage("web-project-02.png"),
    imageAlt: "REAL OR FAKE 풀 페이지 캡쳐 이미지",
  },
  {
    id: "web-project-03",
    title: "산타",
    desc: "등산 정보를 동화적인 분위기로 풀어낸 산행 정보 및 커뮤니티 기반 React 웹사이트입니다.",
    notionUrl: "https://www.notion.so/34751f8c416680ba87c7c07b4a3eef47?source=copy_link",
    siteUrl: "https://santa-app-jw6k.vercel.app/",
    image: getWebImage("web-project-03.png"),
    imageAlt: "산타 풀 페이지 캡쳐 이미지",
  },
  {
    id: "web-project-04",
    title: "Nail Tip",
    desc: "네일 팁 상품을 감성적인 무드로 보여주는 React 기반 네일 아카이브 웹사이트입니다.",
    notionUrl: "https://www.notion.so/35151f8c4166809a8c79d1477dd0a48f?source=copy_link",
    siteUrl: "https://miso6619-sketch.github.io/nail-archive/",
    image: getWebImage("web-project-04.png"),
    imageAlt: "Nail Tip 풀 페이지 캡쳐 이미지",
  },
];